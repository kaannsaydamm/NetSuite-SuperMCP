import { z } from "zod"
import {
  CommitOperationRequestSchema,
  GenericTransformOperationInputSchema,
  PrepareCompensationRequestSchema,
  PrepareOperationRequestSchema,
  PreviewOperationRequestSchema,
  PurchaseOrderOperationInputSchema,
  SalesOrderOperationInputSchema,
} from "../contracts/operation-schemas"
import {
  InventoryAdjustmentAccountSearchRequestSchema,
  InventoryStockImportPrepareRequestSchema,
  RecordCreateRequestSchema,
  RecordDeleteRequestSchema,
  RecordMetadataRequestSchema,
  RecordRefSchema,
  RecordUpdateRequestSchema,
  SuiteQlRequestSchema,
  TransactionLinesRequestSchema,
} from "../netsuite/types"
import { type ToolPolicy, ToolRisk } from "../policy"
import { JsonValueSchema } from "../shared/json"

export const ToolName = {
  GetEnvironment: "ns_getEnvironment",
  GetSuperMcpVersion: "ns_getSuperMcpVersion",
  CheckAccountPermissions: "ns_checkAccountPermissions",
  GetRecord: "ns_getRecord",
  RunSuiteQl: "ns_runSuiteQL",
  RunSavedSearch: "ns_runSavedSearch",
  RunReport: "ns_runReport",
  GetRecordMetadata: "ns_getRecordMetadata",
  GetTransactionLines: "ns_getTransactionLines",
  ListPlatformObjects: "ns_listPlatformObjects",
  GetPlatformObject: "ns_getPlatformObject",
  SearchRecords: "ns_searchRecords",
  ListReportTypes: "ns_listReportTypes",
  ListReports: "ns_listReports",
  RunSearch: "ns_runSearch",
  CreateSavedSearch: "ns_createSavedSearch",
  UpdateSavedSearch: "ns_updateSavedSearch",
  DeleteSavedSearch: "ns_deleteSavedSearch",
  ListFileCabinet: "ns_listFileCabinet",
  GetFile: "ns_getFile",
  WriteFile: "ns_writeFile",
  CreateFolder: "ns_createFolder",
  UpdateFolder: "ns_updateFolder",
  DeleteFolder: "ns_deleteFolder",
  CopyFile: "ns_copyFile",
  MoveFile: "ns_moveFile",
  DeleteFile: "ns_deleteFile",
  GetIntegrationLogs: "ns_getIntegrationLogs",
  GetScriptLogs: "ns_getScriptLogs",
  FindScriptErrors: "ns_findScriptErrors",
  ListScripts: "ns_listScripts",
  ListScriptDeployments: "ns_listScriptDeployments",
  CreateRecord: "ns_createRecord",
  UpdateRecord: "ns_updateRecord",
  SubmitFields: "ns_submitFields",
  DeleteRecord: "ns_deleteRecord",
  TransformRecord: "ns_transformRecord",
  FulfillSalesOrder: "ns_fulfillSalesOrder",
  InvoiceSalesOrder: "ns_invoiceSalesOrder",
  ReceivePurchaseOrder: "ns_receivePurchaseOrder",
  BillPurchaseOrder: "ns_billPurchaseOrder",
  GetFailedIntegrationJobs: "ns_getFailedIntegrationJobs",
  ExplainIntegrationError: "ns_explainIntegrationError",
  RetryIntegrationJob: "ns_retryIntegrationJob",
  GetMapping: "ns_getMapping",
  UpdateMapping: "ns_updateMapping",
  FindInventoryAdjustmentAccounts: "ns_findInventoryAdjustmentAccounts",
  PrepareInventoryStockImport: "ns_prepareInventoryStockImport",
  CommitInventoryStockImport: "ns_commitInventoryStockImport",
  PrepareAction: "ns_prepareAction",
  PreviewAction: "ns_previewAction",
  CommitAction: "ns_commitAction",
  PrepareCompensation: "ns_prepareCompensation",
  GetAuditLog: "ns_getAuditLog",
  ListCapabilities: "ns_listCapabilities",
  DescribeTool: "ns_describeTool",
  GetToolExample: "ns_getToolExample",
  ValidateToolRequest: "ns_validateToolRequest",
  GetLoginAuditTrail: "ns_getLoginAuditTrail",
  DiagnoseAuthentication: "ns_diagnoseAuthentication",
  TestOAuthCredentials: "ns_testOAuthCredentials",
  GetOAuthTokenMetadata: "ns_getOAuthTokenMetadata",
  RevokeOAuthAuthorization: "ns_revokeOAuthAuthorization",
  AnalyzeRoleAccess: "ns_analyzeRoleAccess",
  CompareRoleVisibility: "ns_compareRoleVisibility",
  ExplainTokenEligibility: "ns_explainTokenEligibility",
  GetIdentityRelationship: "ns_getIdentityRelationship",
  GetIntegrationState: "ns_getIntegrationState",
  AnalyzeSegregationOfDuties: "ns_analyzeSegregationOfDuties",
  ListRecordTypes: "ns_listRecordTypes",
  DescribeRecordType: "ns_describeRecordType",
  ListRecordFields: "ns_listRecordFields",
  DescribeField: "ns_describeField",
  FindFieldByLabel: "ns_findFieldByLabel",
  FindRecordByExternalId: "ns_findRecordByExternalId",
  BatchResolveInternalIds: "ns_batchResolveInternalIds",
  BatchGetRecords: "ns_batchGetRecords",
  GetRecordWithSublists: "ns_getRecordWithSublists",
  GetTransactionChain: "ns_getTransactionChain",
  GetSystemNotes: "ns_getSystemNotes",
  ExplainRecordHistory: "ns_explainRecordHistory",
  GetTransactionEventStream: "ns_getTransactionEventStream",
  DiagnoseTransaction: "ns_diagnoseTransaction",
  CreateRecordSnapshot: "ns_createRecordSnapshot",
  DiffRecordSnapshots: "ns_diffRecordSnapshots",
  CreateEvidenceBundle: "ns_createEvidenceBundle",
  BuildSuiteQl: "ns_buildSuiteQL",
  ValidateSuiteQl: "ns_validateSuiteQL",
  ExplainSuiteQl: "ns_explainSuiteQL",
  RunSuiteQlPaged: "ns_runSuiteQLPaged",
  CreateReadJob: "ns_createReadJob",
  GetJobStatus: "ns_getJobStatus",
  RunJobStep: "ns_runJobStep",
  CancelJob: "ns_cancelJob",
  ResumeJob: "ns_resumeJob",
  IncrementalExport: "ns_incrementalExport",
  ExportSuiteQl: "ns_exportSuiteQL",
  ExportSavedSearch: "ns_exportSavedSearch",
  ExportSavedSearchDefinition: "ns_exportSavedSearchDefinition",
  DiffSavedSearchDefinitions: "ns_diffSavedSearchDefinitions",
  PreviewCloneSavedSearch: "ns_previewCloneSavedSearch",
  GetScriptObservability: "ns_getScriptObservability",
  AnalyzeScript: "ns_analyzeScript",
  FindScriptDependencies: "ns_findScriptDependencies",
  FindRecordWriters: "ns_findRecordWriters",
  FindRecordReaders: "ns_findRecordReaders",
  FindFieldUsage: "ns_findFieldUsage",
  FindDuplicateScriptLogic: "ns_findDuplicateScriptLogic",
  GetIntegrationHealth: "ns_getIntegrationHealth",
  DefineIntegrationContract: "ns_defineIntegrationContract",
  ValidateIntegrationContract: "ns_validateIntegrationContract",
  ReconcileRecords: "ns_reconcileRecords",
  ReconcileOrders: "ns_reconcileOrders",
  ReconcileInventory: "ns_reconcileInventory",
  ReconcileReturns: "ns_reconcileReturns",
  ReconcilePayments: "ns_reconcilePayments",
  ShadowPayload: "ns_shadowPayload",
  ReplayPayload: "ns_replayPayload",
  PrepareCanary: "ns_prepareCanary",
  MonitorCanary: "ns_monitorCanary",
  PromoteCanary: "ns_promoteCanary",
  AbortCanary: "ns_abortCanary",
  GenerateSyntheticTransactions: "ns_generateSyntheticTransactions",
  AnonymizePayload: "ns_anonymizePayload",
  GenerateRegressionTests: "ns_generateRegressionTests",
  RunRegressionTests: "ns_runRegressionTests",
  SubscribeIntegrationEvents: "ns_subscribeIntegrationEvents",
  EmitIntegrationEvent: "ns_emitIntegrationEvent",
  PollIntegrationOutbox: "ns_pollIntegrationOutbox",
  AckIntegrationEvent: "ns_ackIntegrationEvent",
  InventoryCustomizations: "ns_inventoryCustomizations",
  DiffCustomizationEnvironments: "ns_diffCustomizationEnvironments",
  GenerateSuiteCloudProject: "ns_generateSuiteCloudProject",
  ValidateSuiteCloudProject: "ns_validateSuiteCloudProject",
  PreviewCustomizationDeployment: "ns_previewCustomizationDeployment",
  PrepareCustomizationDeployment: "ns_prepareCustomizationDeployment",
  GetCustomizationDeployment: "ns_getCustomizationDeployment",
  RecordCustomizationDeploymentResult: "ns_recordCustomizationDeploymentResult",
  VerifyCustomizationDeployment: "ns_verifyCustomizationDeployment",
  PrepareCustomizationRollback: "ns_prepareCustomizationRollback",
  PlanCustomizationMigration: "ns_planCustomizationMigration",
  PlanCustomizationCleanup: "ns_planCustomizationCleanup",
  GenerateSystemDocumentation: "ns_generateSystemDocumentation",
  DefineBusinessTerm: "ns_defineBusinessTerm",
  DefineMetric: "ns_defineMetric",
  DeleteBusinessTerm: "ns_deleteBusinessTerm",
  DeleteMetric: "ns_deleteMetric",
  GetMetricDefinition: "ns_getMetricDefinition",
  PlanBusinessQuery: "ns_planBusinessQuery",
  ValidateMetricPlan: "ns_validateMetricPlan",
  RunMetric: "ns_runMetric",
  CompareMetricDefinitions: "ns_compareMetricDefinitions",
  TraceMetricLineage: "ns_traceMetricLineage",
  GenerateMetricReport: "ns_generateMetricReport",
  ExportMetricResult: "ns_exportMetricResult",
  DiscoverProcess: "ns_discoverProcess",
  DiscoverBusinessRules: "ns_discoverBusinessRules",
  AnalyzeFieldWriteConflicts: "ns_analyzeFieldWriteConflicts",
  ProfileDataQuality: "ns_profileDataQuality",
  ValidateMasterData: "ns_validateMasterData",
  EvaluateInvariants: "ns_evaluateInvariants",
  EvaluatePolicyFacts: "ns_evaluatePolicyFacts",
  SimulateDownstreamImpact: "ns_simulateDownstreamImpact",
  PreviewGlImpact: "ns_previewGlImpact",
  SimulateInventoryState: "ns_simulateInventoryState",
  SimulateChannelAllocation: "ns_simulateChannelAllocation",
  RankRootCauses: "ns_rankRootCauses",
  DefineRunbook: "ns_defineRunbook",
  PreviewRunbook: "ns_previewRunbook",
  StartRunbook: "ns_startRunbook",
  GetRunbookExecution: "ns_getRunbookExecution",
  RecordRunbookStep: "ns_recordRunbookStep",
  ProposeRepair: "ns_proposeRepair",
  PrepareBoundedRepair: "ns_prepareBoundedRepair",
  CorrelateIncidents: "ns_correlateIncidents",
  MeasureSla: "ns_measureSla",
  BuildSupportEvidenceBundle: "ns_buildSupportEvidenceBundle",
  GenerateLiveDocumentation: "ns_generateLiveDocumentation",
  RecordEvidenceClaim: "ns_recordEvidenceClaim",
  GetEvidenceMemory: "ns_getEvidenceMemory",
  CreateCompositeTool: "ns_createCompositeTool",
  GetCompositeTool: "ns_getCompositeTool",
} as const

export type ToolName = (typeof ToolName)[keyof typeof ToolName]

export const toolPolicies = {
  [ToolName.GetEnvironment]: low(ToolName.GetEnvironment),
  [ToolName.GetSuperMcpVersion]: low(ToolName.GetSuperMcpVersion),
  [ToolName.CheckAccountPermissions]: low(ToolName.CheckAccountPermissions),
  [ToolName.GetRecord]: low(ToolName.GetRecord),
  [ToolName.RunSuiteQl]: low(ToolName.RunSuiteQl),
  [ToolName.RunSavedSearch]: low(ToolName.RunSavedSearch),
  [ToolName.RunReport]: low(ToolName.RunReport),
  [ToolName.GetRecordMetadata]: low(ToolName.GetRecordMetadata),
  [ToolName.GetTransactionLines]: low(ToolName.GetTransactionLines),
  [ToolName.ListPlatformObjects]: low(ToolName.ListPlatformObjects),
  [ToolName.GetPlatformObject]: low(ToolName.GetPlatformObject),
  [ToolName.SearchRecords]: low(ToolName.SearchRecords),
  [ToolName.ListReportTypes]: low(ToolName.ListReportTypes),
  [ToolName.ListReports]: low(ToolName.ListReports),
  [ToolName.RunSearch]: low(ToolName.RunSearch),
  [ToolName.CreateSavedSearch]: medium(ToolName.CreateSavedSearch),
  [ToolName.UpdateSavedSearch]: medium(ToolName.UpdateSavedSearch),
  [ToolName.DeleteSavedSearch]: high(ToolName.DeleteSavedSearch),
  [ToolName.ListFileCabinet]: low(ToolName.ListFileCabinet),
  [ToolName.GetFile]: low(ToolName.GetFile),
  [ToolName.WriteFile]: high(ToolName.WriteFile),
  [ToolName.CreateFolder]: medium(ToolName.CreateFolder),
  [ToolName.UpdateFolder]: medium(ToolName.UpdateFolder),
  [ToolName.DeleteFolder]: high(ToolName.DeleteFolder),
  [ToolName.CopyFile]: medium(ToolName.CopyFile),
  [ToolName.MoveFile]: medium(ToolName.MoveFile),
  [ToolName.DeleteFile]: high(ToolName.DeleteFile),
  [ToolName.GetIntegrationLogs]: low(ToolName.GetIntegrationLogs),
  [ToolName.GetScriptLogs]: low(ToolName.GetScriptLogs),
  [ToolName.FindScriptErrors]: low(ToolName.FindScriptErrors),
  [ToolName.ListScripts]: low(ToolName.ListScripts),
  [ToolName.ListScriptDeployments]: low(ToolName.ListScriptDeployments),
  [ToolName.GetFailedIntegrationJobs]: low(ToolName.GetFailedIntegrationJobs),
  [ToolName.ExplainIntegrationError]: low(ToolName.ExplainIntegrationError),
  [ToolName.GetMapping]: low(ToolName.GetMapping),
  [ToolName.GetAuditLog]: low(ToolName.GetAuditLog),
  [ToolName.ListCapabilities]: low(ToolName.ListCapabilities),
  [ToolName.DescribeTool]: low(ToolName.DescribeTool),
  [ToolName.GetToolExample]: low(ToolName.GetToolExample),
  [ToolName.ValidateToolRequest]: low(ToolName.ValidateToolRequest),
  [ToolName.GetLoginAuditTrail]: low(ToolName.GetLoginAuditTrail),
  [ToolName.DiagnoseAuthentication]: low(ToolName.DiagnoseAuthentication),
  [ToolName.TestOAuthCredentials]: low(ToolName.TestOAuthCredentials),
  [ToolName.GetOAuthTokenMetadata]: low(ToolName.GetOAuthTokenMetadata),
  [ToolName.RevokeOAuthAuthorization]: diagnosticHigh(ToolName.RevokeOAuthAuthorization),
  [ToolName.AnalyzeRoleAccess]: low(ToolName.AnalyzeRoleAccess),
  [ToolName.CompareRoleVisibility]: low(ToolName.CompareRoleVisibility),
  [ToolName.ExplainTokenEligibility]: low(ToolName.ExplainTokenEligibility),
  [ToolName.GetIdentityRelationship]: low(ToolName.GetIdentityRelationship),
  [ToolName.GetIntegrationState]: low(ToolName.GetIntegrationState),
  [ToolName.AnalyzeSegregationOfDuties]: low(ToolName.AnalyzeSegregationOfDuties),
  [ToolName.ListRecordTypes]: low(ToolName.ListRecordTypes),
  [ToolName.DescribeRecordType]: low(ToolName.DescribeRecordType),
  [ToolName.ListRecordFields]: low(ToolName.ListRecordFields),
  [ToolName.DescribeField]: low(ToolName.DescribeField),
  [ToolName.FindFieldByLabel]: low(ToolName.FindFieldByLabel),
  [ToolName.FindRecordByExternalId]: low(ToolName.FindRecordByExternalId),
  [ToolName.BatchResolveInternalIds]: low(ToolName.BatchResolveInternalIds),
  [ToolName.BatchGetRecords]: low(ToolName.BatchGetRecords),
  [ToolName.GetRecordWithSublists]: low(ToolName.GetRecordWithSublists),
  [ToolName.GetTransactionChain]: low(ToolName.GetTransactionChain),
  [ToolName.GetSystemNotes]: low(ToolName.GetSystemNotes),
  [ToolName.ExplainRecordHistory]: low(ToolName.ExplainRecordHistory),
  [ToolName.GetTransactionEventStream]: low(ToolName.GetTransactionEventStream),
  [ToolName.DiagnoseTransaction]: low(ToolName.DiagnoseTransaction),
  [ToolName.CreateRecordSnapshot]: low(ToolName.CreateRecordSnapshot),
  [ToolName.DiffRecordSnapshots]: low(ToolName.DiffRecordSnapshots),
  [ToolName.CreateEvidenceBundle]: low(ToolName.CreateEvidenceBundle),
  [ToolName.BuildSuiteQl]: low(ToolName.BuildSuiteQl),
  [ToolName.ValidateSuiteQl]: low(ToolName.ValidateSuiteQl),
  [ToolName.ExplainSuiteQl]: low(ToolName.ExplainSuiteQl),
  [ToolName.RunSuiteQlPaged]: low(ToolName.RunSuiteQlPaged),
  [ToolName.CreateReadJob]: low(ToolName.CreateReadJob),
  [ToolName.GetJobStatus]: low(ToolName.GetJobStatus),
  [ToolName.RunJobStep]: low(ToolName.RunJobStep),
  [ToolName.CancelJob]: low(ToolName.CancelJob),
  [ToolName.ResumeJob]: low(ToolName.ResumeJob),
  [ToolName.IncrementalExport]: low(ToolName.IncrementalExport),
  [ToolName.ExportSuiteQl]: low(ToolName.ExportSuiteQl),
  [ToolName.ExportSavedSearch]: low(ToolName.ExportSavedSearch),
  [ToolName.ExportSavedSearchDefinition]: low(ToolName.ExportSavedSearchDefinition),
  [ToolName.DiffSavedSearchDefinitions]: low(ToolName.DiffSavedSearchDefinitions),
  [ToolName.PreviewCloneSavedSearch]: low(ToolName.PreviewCloneSavedSearch),
  [ToolName.GetScriptObservability]: low(ToolName.GetScriptObservability),
  [ToolName.AnalyzeScript]: low(ToolName.AnalyzeScript),
  [ToolName.FindScriptDependencies]: low(ToolName.FindScriptDependencies),
  [ToolName.FindRecordWriters]: low(ToolName.FindRecordWriters),
  [ToolName.FindRecordReaders]: low(ToolName.FindRecordReaders),
  [ToolName.FindFieldUsage]: low(ToolName.FindFieldUsage),
  [ToolName.FindDuplicateScriptLogic]: low(ToolName.FindDuplicateScriptLogic),
  [ToolName.GetIntegrationHealth]: low(ToolName.GetIntegrationHealth),
  [ToolName.DefineIntegrationContract]: low(ToolName.DefineIntegrationContract),
  [ToolName.ValidateIntegrationContract]: low(ToolName.ValidateIntegrationContract),
  [ToolName.ReconcileRecords]: low(ToolName.ReconcileRecords),
  [ToolName.ReconcileOrders]: low(ToolName.ReconcileOrders),
  [ToolName.ReconcileInventory]: low(ToolName.ReconcileInventory),
  [ToolName.ReconcileReturns]: low(ToolName.ReconcileReturns),
  [ToolName.ReconcilePayments]: low(ToolName.ReconcilePayments),
  [ToolName.ShadowPayload]: low(ToolName.ShadowPayload),
  [ToolName.ReplayPayload]: low(ToolName.ReplayPayload),
  [ToolName.PrepareCanary]: low(ToolName.PrepareCanary),
  [ToolName.MonitorCanary]: low(ToolName.MonitorCanary),
  [ToolName.PromoteCanary]: low(ToolName.PromoteCanary),
  [ToolName.AbortCanary]: low(ToolName.AbortCanary),
  [ToolName.GenerateSyntheticTransactions]: low(ToolName.GenerateSyntheticTransactions),
  [ToolName.AnonymizePayload]: low(ToolName.AnonymizePayload),
  [ToolName.GenerateRegressionTests]: low(ToolName.GenerateRegressionTests),
  [ToolName.RunRegressionTests]: low(ToolName.RunRegressionTests),
  [ToolName.SubscribeIntegrationEvents]: low(ToolName.SubscribeIntegrationEvents),
  [ToolName.EmitIntegrationEvent]: low(ToolName.EmitIntegrationEvent),
  [ToolName.PollIntegrationOutbox]: low(ToolName.PollIntegrationOutbox),
  [ToolName.AckIntegrationEvent]: low(ToolName.AckIntegrationEvent),
  [ToolName.InventoryCustomizations]: low(ToolName.InventoryCustomizations),
  [ToolName.DiffCustomizationEnvironments]: low(ToolName.DiffCustomizationEnvironments),
  [ToolName.GenerateSuiteCloudProject]: low(ToolName.GenerateSuiteCloudProject),
  [ToolName.ValidateSuiteCloudProject]: low(ToolName.ValidateSuiteCloudProject),
  [ToolName.PreviewCustomizationDeployment]: low(ToolName.PreviewCustomizationDeployment),
  [ToolName.PrepareCustomizationDeployment]: low(ToolName.PrepareCustomizationDeployment),
  [ToolName.GetCustomizationDeployment]: low(ToolName.GetCustomizationDeployment),
  [ToolName.RecordCustomizationDeploymentResult]: low(ToolName.RecordCustomizationDeploymentResult),
  [ToolName.VerifyCustomizationDeployment]: low(ToolName.VerifyCustomizationDeployment),
  [ToolName.PrepareCustomizationRollback]: low(ToolName.PrepareCustomizationRollback),
  [ToolName.PlanCustomizationMigration]: low(ToolName.PlanCustomizationMigration),
  [ToolName.PlanCustomizationCleanup]: low(ToolName.PlanCustomizationCleanup),
  [ToolName.GenerateSystemDocumentation]: low(ToolName.GenerateSystemDocumentation),
  [ToolName.DefineBusinessTerm]: low(ToolName.DefineBusinessTerm),
  [ToolName.DefineMetric]: low(ToolName.DefineMetric),
  [ToolName.DeleteBusinessTerm]: low(ToolName.DeleteBusinessTerm),
  [ToolName.DeleteMetric]: low(ToolName.DeleteMetric),
  [ToolName.GetMetricDefinition]: low(ToolName.GetMetricDefinition),
  [ToolName.PlanBusinessQuery]: low(ToolName.PlanBusinessQuery),
  [ToolName.ValidateMetricPlan]: low(ToolName.ValidateMetricPlan),
  [ToolName.RunMetric]: low(ToolName.RunMetric),
  [ToolName.CompareMetricDefinitions]: low(ToolName.CompareMetricDefinitions),
  [ToolName.TraceMetricLineage]: low(ToolName.TraceMetricLineage),
  [ToolName.GenerateMetricReport]: low(ToolName.GenerateMetricReport),
  [ToolName.ExportMetricResult]: low(ToolName.ExportMetricResult),
  [ToolName.DiscoverProcess]: low(ToolName.DiscoverProcess),
  [ToolName.DiscoverBusinessRules]: low(ToolName.DiscoverBusinessRules),
  [ToolName.AnalyzeFieldWriteConflicts]: low(ToolName.AnalyzeFieldWriteConflicts),
  [ToolName.ProfileDataQuality]: low(ToolName.ProfileDataQuality),
  [ToolName.ValidateMasterData]: low(ToolName.ValidateMasterData),
  [ToolName.EvaluateInvariants]: low(ToolName.EvaluateInvariants),
  [ToolName.EvaluatePolicyFacts]: low(ToolName.EvaluatePolicyFacts),
  [ToolName.SimulateDownstreamImpact]: low(ToolName.SimulateDownstreamImpact),
  [ToolName.PreviewGlImpact]: low(ToolName.PreviewGlImpact),
  [ToolName.SimulateInventoryState]: low(ToolName.SimulateInventoryState),
  [ToolName.SimulateChannelAllocation]: low(ToolName.SimulateChannelAllocation),
  [ToolName.RankRootCauses]: low(ToolName.RankRootCauses),
  [ToolName.DefineRunbook]: low(ToolName.DefineRunbook),
  [ToolName.PreviewRunbook]: low(ToolName.PreviewRunbook),
  [ToolName.StartRunbook]: low(ToolName.StartRunbook),
  [ToolName.GetRunbookExecution]: low(ToolName.GetRunbookExecution),
  [ToolName.RecordRunbookStep]: low(ToolName.RecordRunbookStep),
  [ToolName.ProposeRepair]: low(ToolName.ProposeRepair),
  [ToolName.PrepareBoundedRepair]: low(ToolName.PrepareBoundedRepair),
  [ToolName.CorrelateIncidents]: low(ToolName.CorrelateIncidents),
  [ToolName.MeasureSla]: low(ToolName.MeasureSla),
  [ToolName.BuildSupportEvidenceBundle]: low(ToolName.BuildSupportEvidenceBundle),
  [ToolName.GenerateLiveDocumentation]: low(ToolName.GenerateLiveDocumentation),
  [ToolName.RecordEvidenceClaim]: low(ToolName.RecordEvidenceClaim),
  [ToolName.GetEvidenceMemory]: low(ToolName.GetEvidenceMemory),
  [ToolName.CreateCompositeTool]: low(ToolName.CreateCompositeTool),
  [ToolName.GetCompositeTool]: low(ToolName.GetCompositeTool),
  [ToolName.CreateRecord]: medium(ToolName.CreateRecord),
  [ToolName.UpdateRecord]: medium(ToolName.UpdateRecord),
  [ToolName.SubmitFields]: medium(ToolName.SubmitFields),
  [ToolName.DeleteRecord]: high(ToolName.DeleteRecord),
  [ToolName.UpdateMapping]: medium(ToolName.UpdateMapping),
  [ToolName.FindInventoryAdjustmentAccounts]: low(ToolName.FindInventoryAdjustmentAccounts),
  [ToolName.PrepareInventoryStockImport]: low(ToolName.PrepareInventoryStockImport),
  [ToolName.CommitInventoryStockImport]: high(ToolName.CommitInventoryStockImport),
  [ToolName.TransformRecord]: high(ToolName.TransformRecord),
  [ToolName.FulfillSalesOrder]: high(ToolName.FulfillSalesOrder),
  [ToolName.InvoiceSalesOrder]: high(ToolName.InvoiceSalesOrder),
  [ToolName.ReceivePurchaseOrder]: high(ToolName.ReceivePurchaseOrder),
  [ToolName.BillPurchaseOrder]: high(ToolName.BillPurchaseOrder),
  [ToolName.RetryIntegrationJob]: high(ToolName.RetryIntegrationJob),
  [ToolName.PrepareAction]: low(ToolName.PrepareAction),
  [ToolName.PreviewAction]: low(ToolName.PreviewAction),
  [ToolName.CommitAction]: high(ToolName.CommitAction),
  [ToolName.PrepareCompensation]: low(ToolName.PrepareCompensation),
} satisfies Record<ToolName, ToolPolicy>

export const EmptyInputSchema = z.object({})
export const RecordInputSchema = RecordRefSchema
export const RecordCreateInputSchema = RecordCreateRequestSchema
export const RecordUpdateInputSchema = RecordUpdateRequestSchema
export const RecordDeleteInputSchema = RecordDeleteRequestSchema
export const RecordDeletePlanInputSchema = RecordRefSchema
export const RecordMetadataInputSchema = RecordMetadataRequestSchema
export const TransactionLinesInputSchema = TransactionLinesRequestSchema
export const SuiteQlInputSchema = SuiteQlRequestSchema
export const AuditLogInputSchema = z.object({
  limit: z.number().int().min(1).max(100).default(20),
})

export const AccountPermissionCheckInputSchema = z.object({
  recordTypes: z.array(z.string().min(1)).max(20).default([]),
  includeRestlet: z.boolean().default(false),
})

export const ToolContractLookupInputSchema = z.object({ name: z.string().min(1) })
export const ToolRequestValidationInputSchema = z.object({
  name: z.string().min(1),
  payload: JsonValueSchema,
})

export const PrepareOperationInputSchema = PrepareOperationRequestSchema
export const PreviewOperationInputSchema = PreviewOperationRequestSchema
export const CommitOperationInputSchema = CommitOperationRequestSchema
export const PrepareCompensationInputSchema = PrepareCompensationRequestSchema
export {
  GenericTransformOperationInputSchema,
  PurchaseOrderOperationInputSchema,
  SalesOrderOperationInputSchema,
}
export const InventoryAdjustmentAccountSearchInputSchema =
  InventoryAdjustmentAccountSearchRequestSchema
export const InventoryStockImportPrepareInputSchema = InventoryStockImportPrepareRequestSchema
export const InventoryStockImportCommitInputSchema = InventoryStockImportPrepareRequestSchema

function low(toolName: ToolName): ToolPolicy {
  return { toolName, risk: ToolRisk.Low, mutatesNetSuite: false }
}

function medium(toolName: ToolName): ToolPolicy {
  return { toolName, risk: ToolRisk.Medium, mutatesNetSuite: true }
}

function high(toolName: ToolName): ToolPolicy {
  return { toolName, risk: ToolRisk.High, mutatesNetSuite: true }
}

function diagnosticHigh(toolName: ToolName): ToolPolicy {
  return { toolName, risk: ToolRisk.High, mutatesNetSuite: false }
}
